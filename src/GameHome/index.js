...
class GameHome extends Component {
  ...
  matchImage = iD => {
    const {id, timerId} = this.state
    if (id === iD) {
      this.setState(prevState => ({
        score: prevState.score + 1,
      }))
      const val = Math.floor(Math.random() * 30)
      const changeUrl = imagesList[val].imageUrl
      const imageId = imagesList[val].id
      this.setState({url: changeUrl, id: imageId})
    } else if (id !== iD) {
      this.setState({timerUp: false})
      clearInterval(timerId)
    }
  }

  play = () => {
    this.setState({score: 0, timer: 60, timerUp: true})
    this.tick()
  }
  ...
  render() {
    const {score, timer, activeTabId, url, timerUp} = this.state

    if (timerUp === true && timer === 0) {
      this.checkTime()
    }

    const activeImagesList = imagesList.filter(
      eachItem => eachItem.category === activeTabId,
    )

    return (
      <div>
        <div>{this.Header(score, timer)}</div>
        <div className="game-container">
          {timerUp ? (
            <div className="match-img-container">
              <img src={url} alt="match" className="match" />
            ...
            <ScoreCard score={score} play={this.play} />
          )}
        </div>
      </div>
    )
